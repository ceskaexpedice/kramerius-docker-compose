...
export STATE=""
export ORGANIZATION=""
export COMMON_NAME="$CDK_HOSTNAME"
export ALT_NAMES="DNS:$CDK_HOSTNAME"
...
#if [ ! -f "$CERT_DIR/$CDK_HOSTNAME.crt" ] && [ ! -f "$CERT_DIR/$CDK_HOSTNAME.key" ]; then
if [ ! -f "/usr/local/etc/ssl/hostname.txt" ] || [ "$(cat /usr/local/etc/ssl/hostname.txt)" != "$CDK_HOSTNAME" ]; then
	...
	#Zip file 
	zip -j "$ZIP_DIR/certificates.zip" "$CLIENT_DIR/ceskadigitalniknihovna.cz.crt" "$CLIENT_DIR/ceskadigitalniknihovna.cz.key" "$CERT_DIR/$CDK_HOSTNAME.crt"

	#Mkdir 
    mkdir "/usr/local/apache2/conf/$CDK_HOSTNAME"
	
	#Configuration
	printf "<VirtualHost *:80>\n\
    ServerAdmin admin@$CDK_HOSTNAME\n\
    ServerName $CDK_HOSTNAME\n\
    ServerAlias $CDK_HOSTNAME\n\
    DocumentRoot /var/www/$CDK_HOSTNAME\n\
    ErrorLog \${APACHE_LOG_DIR}/error.log\n\
    CustomLog \${APACHE_LOG_DIR}/access.log combined\n\
	\n\
    RewriteEngine on\n\
    RewriteCond %%{SERVER_NAME} =$CDK_HOSTNAME\n\
    RewriteRule ^ https://%%{SERVER_NAME}%%{REQUEST_URI} [END,NE,R=permanent]\n\
	</VirtualHost>\n" > "/usr/local/apache2/conf/$CDK_HOSTNAME/$CDK_HOSTNAME.conf"		
    ...
    SSLVerifyDepth 10\n\
	\n\
    SSLCertificateFile /TODO_PATH/ssl/certs/$CDK_HOSTNAME.crt\n\
    SSLCertificateKeyFile /TODO_PATH/ssl/certs/$CDK_HOSTNAME.key\n\
